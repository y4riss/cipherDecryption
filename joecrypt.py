...
from art import *
if(len(sys.argv) != 7):
    print(f" # USAGE : python {sys.argv[0]} -str yourString -n RotationNumber -d backward/forward")
    exit()

tprint("J O E C R Y P T")

# Construct the argument parser
ap = argparse.ArgumentParser()

# Add the arguments to the parser
ap.add_argument("-str", "--string", required=True,
   help="string to decrypt")
ap.add_argument("-n", "--rotation", required=True,
   help="rotation number")
ap.add_argument("-d", "--direction", required=True,
   help="direction : forward/backward")
...
if(opt != "forward" and opt!= "backward"):
    print(" please select your rotation direction ( forward for going forward / backward for going backwards ) ")
    exit()
elif (opt == "backward"):
        n += 24
